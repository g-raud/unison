...
space.
\begin{alltt}
             ignore = del \ARG{pattern}
\end{alltt}
Any subsequent (positive) pattern can select the path again.  A negative
pattern is similar to a positive pattern in the corresponding negative
preference (one whose name ends with \texttt{not}), except that the matched
paths can be selected again by following (positive) patterns.  However a
negative pattern in a negative preference can only make a path selected
by a positive preference if the path has also been explicitely selected with
